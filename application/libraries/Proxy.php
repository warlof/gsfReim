...
class Proxy { 
	...
	private $_user_agent = 'GSF Reimbursement';
	...
	public function __destruct()
	{
		...
		// if (file_exists('tmp.gz')) unlink('tmp.gz'); $CI->session->userdata('user_authen')
		// /' . $CI->session->userdata('session_id') . '
		// session_id()  tmp' . session_id()
		if (file_exists('tmp/tmp.gz')) unlink('tmp/tmp.gz');
	}
	...
	protected function _execute($url = '', $method = '', $maxredir = 10, $body_params = array(), $head_params = array())
	{
		...
		if (!$fp) 
		{
			...
			{
				show_error($errstr.' [ERROR_CODE] = '.$errno . " | URL: " . $url);
			}
		}
		...
		{/*
			...
			@file_put_contents('tmp/tmp' . $final_rand . '.gz', $body);
			$fp = @gzopen('tmp/tmp' . $final_rand . '.gz', 'r');
			for($body = ''; !@gzeof($fp); $body.=@gzgets($fp, 4096)) {}
			@gzclose($fp);
			$fileName = 'tmp/tmp' . $final_rand . '.gz';
		 * */
		 $body = gzdecode($body);
		}
